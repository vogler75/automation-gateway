...
copy ..\source\cluster\influxdb\build\distributions\iotdb.tar app.tar
docker build --build-arg APP_NAME=iotdb -t frankenstein-iotdb .
del app.tar

copy ..\source\cluster\influxdb\build\distributions\kafka.tar app.tar
docker build --build-arg APP_NAME=kafka -t frankenstein-kafka .
del app.tar
