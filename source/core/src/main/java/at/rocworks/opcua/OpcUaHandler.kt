...
import at.rocworks.data.Topic.TopicType
...
class OpcUaHandler(config: JsonObject) : OpcUaVerticle(config) {
    ...
    private fun valueConsumer(topic: Topic, item: UaMonitoredItem, data: DataValue) {
        ...
        try {
            ...
            val data : Buffer? = when (topic.format) {
                Topic.Format.Value -> {
            ...
            if (data!=null) vertx.eventBus().publish(topic.topicName, data)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
